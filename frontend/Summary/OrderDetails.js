...
//       alert("Please select at least one item with a wash type.");
//       return;
//     }
...
//     (item) => item.quantity > 0 && item.selectedWashes.length > 0
//   );
...
const OrderTable = () => {
  ...
  return (
    <div className="order-container">
      ...
      <div className="actions">
        <button className="cancel" onClick={() => navigate("/")}>Cancel</button>
        <button className="proceed" onClick={goToSummary} disabled={!hasValidSelection}>
          Proceed
        </button>
      </div>
    </div>
